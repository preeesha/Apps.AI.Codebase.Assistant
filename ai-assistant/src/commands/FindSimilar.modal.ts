import { IHttp, IModify, IRead, IUIKitSurfaceViewParam } from "@rocket.chat/apps-engine/definition/accessors"
...
async function process(http: IHttp, query: string): Promise<DBNode[] | null> {
   ...
   const similarNodes = await Query.getDBNodesFromVectorQuery(db, "codeEmbeddings", queryVector, 0.5)
...
export async function findSimilarModalSubmitHandler(
   ...
   if (!res) {
      await sendMessage("❌ Failed to find similar chunks. Please try again later.")
      return
   }
   if (!res.length) {
